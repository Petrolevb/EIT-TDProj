...
namespace TP4_FreqBayes
{
    class Twit
    {
        ...
        public Twit(string contenu)
        {
            ...
            foreach (string mot in this.m_Contenu.Split(' '))
            {
                ...
                string newMot = mot.Replace(",","").Replace(";","").Replace(":","")
                                    .Replace(".","").ToLower();
                // replace #mots by HASHTAG
                if (newMot.StartsWith("#")) newMot = "HASHTAG";

                if (!this.m_NbMots.ContainsKey(newMot))
                    this.m_NbMots.Add(newMot, 0);
                this.m_NbMots[newMot]++;
            }
        }
        ...
        public bool IsWordIn(string word) { return this.m_NbMots.ContainsKey(word); }
        public int NumberWordIn(string word) { return (this.m_NbMots.ContainsKey(word) ? this.m_NbMots[word] : 0); }
        public List<String> Words { get { return this.m_NbMots.Keys.ToList(); } }
