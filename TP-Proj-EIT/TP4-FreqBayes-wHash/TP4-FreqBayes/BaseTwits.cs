...
namespace TP4_FreqBayes
{
    class BaseTwits
    {
        ...
        public string checkTwit(Twit t)
        {
            ...
            foreach (string word in t.Words)
            {
                if (this.m_BetasPositive.ContainsKey(word))
                    probaPositive += t.NumberWordIn(word) * Math.Log(this.m_BetasPositive[word]);
                if (this.m_BetasNegative.ContainsKey(word))
                    probaNegative += t.NumberWordIn(word) * Math.Log(this.m_BetasNegative[word]);
                if (this.m_BetasNeutral.ContainsKey(word))
                probaNeutral += t.NumberWordIn(word) * Math.Log(this.m_BetasNeutral[word]);
                if (this.m_BetasIrrelevant.ContainsKey(word))
                probaIrrelevant += t.NumberWordIn(word) * Math.Log(this.m_BetasIrrelevant[word]);
            }
        ...
        public void InitBase(double k)
        {
            double[] numberInCategories = { 0,0,0,0 };
            foreach (Twit t in this.m_Twits)
                foreach(string word in t.Words)
                {
                    ...
                    {
                        case "positive":    
                            this.m_BetasPositive[word]+= t.NumberWordIn(word);
                            numberInCategories[0] += t.NumberWordIn(word);
                            break;
                        case "negative": 
                            this.m_BetasNegative[word] += t.NumberWordIn(word);
                            numberInCategories[1] += t.NumberWordIn(word);
                            break;
                        case "neutral": 
                            this.m_BetasNeutral[word] += t.NumberWordIn(word);
                            numberInCategories[2] += t.NumberWordIn(word);
                            break; ;
                        case "irrelevant": 
                            this.m_BetasIrrelevant[word] += t.NumberWordIn(word);
                            numberInCategories[3] += t.NumberWordIn(word);
                            break;
                    }
            ...
            double sommeBetasPositif = 0,
                   sommeBetasNegatif = 0,
                   sommeBetasNeutral = 0,
                   sommeBetasIrrelevent = 0;
            foreach (string key in this.m_AllWords.Keys)
            {
                if (this.m_BetasPositive.ContainsKey(key))
                {
                    this.m_BetasPositive[key] =
                        Math.Max(k, this.m_BetasPositive[key]) / numberInCategories[0];
                    sommeBetasPositif += this.m_BetasPositive[key];
                }
                if (this.m_BetasNegative.ContainsKey(key))
                {
                    this.m_BetasNegative[key] =
                        Math.Max(k, this.m_BetasNegative[key]) / numberInCategories[1];
                    sommeBetasNegatif += this.m_BetasNegative[key];
                }
                if (this.m_BetasNeutral.ContainsKey(key))
                {
                    this.m_BetasNeutral[key] =
                        Math.Max(k, this.m_BetasNeutral[key]) / numberInCategories[2];
                    sommeBetasNeutral += this.m_BetasNeutral[key];
                }
                if (this.m_BetasIrrelevant.ContainsKey(key))
                {
                    this.m_BetasIrrelevant[key] =
                        Math.Max(k, this.m_BetasIrrelevant[key]) / numberInCategories[3];
                    sommeBetasIrrelevent += this.m_BetasIrrelevant[key];
                }
            }
            /* Test JUST IF K = 0 !
            if (sommeBetasPositif < 0.99 || sommeBetasPositif > 1.001) throw new Exception("Betas Irr diff 1");
            if (sommeBetasNegatif < 0.99 || sommeBetasNegatif > 1.001) throw new Exception("Betas Irr diff 1");
            if (sommeBetasNeutral < 0.99 || sommeBetasIrrelevent > 1.001) throw new Exception("Betas Irr diff 1");
            if (sommeBetasIrrelevent < 0.99 || sommeBetasIrrelevent > 1.001) throw new Exception("Betas Irr diff 1");
            // */
        }
